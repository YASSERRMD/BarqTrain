...
__global__ void fused_lora_forward_kernel(
    ...
    // Loop over input features (use loop unrolling and vectorization in production)
...
__global__ void fused_lora_forward_tiled_kernel(
    ...
    for (int in_tile = 0; in_tile < in_features; in_tile += TILE_IN) {
        ...
        if (out_idx < out_features && in_idx < in_features) {
            ...
            if constexpr (std::is_same<T, half>::value) {
                W_shared[tile_out_idx * TILE_IN + tile_in_idx] = __half2float(W_base[w_pos]);
            } else if constexpr (std::is_same<T, __nv_bfloat16>::value) {
                W_shared[tile_out_idx * TILE_IN + tile_in_idx] = __bfloat162float(W_base[w_pos]);
            } else {
    ...
    for (int r = 0; r < rank; r++) {
        ...
        // Multiply by B[out_idx, r]
        if (tile_in_idx == 0 && out_idx < out_features) {
            int b_pos = out_idx * rank + r;
            float b_val = 0.0f;
            if constexpr (std::is_same<T, half>::value) {
    ...
    if (batch_idx < batch_size && out_idx < out_features) {
        int out_pos = batch_idx * out_features + out_idx;
...
torch::Tensor fused_lora_forward_cuda(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND2(
        ...
        x.scalar_type(), "fused_lora_forward", ([&] {
            using T = scalar_t;
